 ...
 *
 * 실제 비트코인 1분봉으로 직전 20봉과 닮은 과거를 세어 봤다 (기준 0.85,
 * 통계가 나오려면 20개가 있어야 한다).
 *
 *     7일     10,080봉      1개
 *     30일    43,200봉      3개
 *     90일   129,600봉      8개
 ...
 *     2년                 ~36개   ← 여기서부터 답이 나온다
 *
 * 시험에 쓰는 가짜 시세는 사인파라 10,080봉이면 160번쯤 반복된다. 그래서
 * 7일치로도 닮은 과거가 넘쳐났고, 나는 그걸 보고 7일을 기본값으로 뒀다.
 * **진짜 시장은 그렇게 반복하지 않는다.** 처음 온 사람이 눌러 보면
 * "판단할 수 없습니다"만 봤을 것이다.
 *
 * 7일을 골랐던 이유는 '받는 데 오래 걸려서'였는데, 이제 미리 받아 둔
 * 파일에서 읽으므로 그 이유가 없어졌다 — 1년치가 한 번에 2.6MB다.
 *
 * 2년이 아니라 1년으로 둔 이유: 2년은 105만 봉이라 아이패드에서 메모리를
 * 75MB쯤 쓴다. 1년(38MB)이 안전한 선이고, 모자라면 화면이 늘리라고 말한다.
 */
export const DEFAULT_PERIOD = 525600;
...
export function whyNothingMatched(rows) {
  ...
  //
  // 직전 봉이 이미 20~40이면 그걸 더 줄이라는 말은 아무 소용이 없다.
  // 그때 듣는 것은 기간이다 — 진짜 데이터로 재 보면 닮은 과거가 기간에
  // 거의 비례해서 는다 (7일 1개 → 30일 3개 → 90일 8개 → 1년 18개).
  const advice = [];
  if (length >= 60) {
    advice.push(`**직전 몇 개 봉**이 ${length}개입니다. 20~40으로 줄여 보세요 — `
      + '긴 모양이 과거에 그대로 반복될 일은 드물어서, 유사도를 낮추는 것보다 '
      + '이쪽이 훨씬 크게 듣습니다.');
    advice.push('그래도 모자라면 **얼마나 과거까지**를 늘리세요.');
  } else {
    advice.push('**얼마나 과거까지**를 늘려 보세요 — 닮은 과거는 기간에 거의 '
      + '비례해서 늘어납니다 (실제 비트코인 1분봉 기준 7일 1개 → 1년 18개 → 2년 36개).');
    advice.push('**얼마나 닮아야 같은 모양인가**를 0.80쯤으로 낮추면 표본이 크게 늡니다. '
